...
def get_location_from_ip(ip_address: str) -> str:
    ...
    try:
        ...
        if response.status_code != 200:
            return f"Error: Could not retrieve location information. Status code: {response.status_code}"
        ...
        if data.get("status") == "fail":
            ...
            return f"Error retrieving location: {error_message}"
        ...
        return "Error: Request timed out while retrieving location information."
    except requests.exceptions.RequestException as e:
        return f"Error: Could not retrieve location information. {str(e)}"
