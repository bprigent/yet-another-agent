"""Tool to get sent emails between two dates with structured results."""
...
from tools.schemas import SentEmailsByDateRangeInput
...
@log_tool_call("get_sent_emails_by_date_range")
def get_sent_emails_by_date_range(
    start_date: str,
    end_date: str,
    max_results: int = 50
) -> ToolResult:
    """Get sent emails between two dates with their ID, subject line, and recipient details.
    
    Use this tool when the user asks about emails they sent during a specific time period,
    wants to search sent emails by date range, or needs to find emails they sent on certain dates.
    ...
    Returns:
        Formatted string with list of sent emails including:
        - Email ID (for use with other email tools)
        - Subject line
        - Recipient name and email address (To field)
        - Date sent
    """
    try:
        # Validate input
        try:
            input_data = SentEmailsByDateRangeInput(
        ...
        # Build Gmail query with date range for SENT emails
        ...
        try:
            # List messages from SENT folder with date range query
            results = service.users().messages().list(
                userId='me',
                labelIds=['SENT'],  # Use SENT label instead of INBOX
                q=query,
            ...
            if not messages:
                return ToolResult(
                    ...
                    data={
                        "emails": [],
                        "count": 0,
                        "formatted": f"No sent emails found between {start_date} and {end_date}."
                    },
                    metadata={
                        "start_date": start_date,
                        "end_date": end_date,
            ...
            for msg in messages:
                ...
                try:
                    message = service.users().messages().get(
                        ...
                        id=msg_id,
                        format='metadata',
                        metadataHeaders=['To', 'Subject', 'Date', 'Cc', 'Bcc']
                    ).execute()
                    ...
                    # Parse recipient name and email (To field)
                    recipient_name, recipient_email = parseaddr(to_header)
                    if not recipient_name:
                        recipient_name = recipient_email.split('@')[0] if recipient_email else 'Unknown'
                    
                    # Format email entry
                    email_entry = f"Email ID: {msg_id}\n"
                    email_entry += f"  Subject: {subject}\n"
                    email_entry += f"  To: {recipient_name} <{recipient_email}>\n"
                    if cc_header:
                        email_entry += f"  CC: {cc_header}\n"
                    if bcc_header:
                        email_entry += f"  BCC: {bcc_header}\n"
                    email_entry += f"  Date Sent: {date}\n"
            ...
            formatted_result = f"Found {len(email_list)} sent email(s) between {start_date} and {end_date}:\n\n"
            formatted_result += "\n".join(email_list)
            ...
            return ToolResult(
                success=False,
                error=f"Error retrieving sent emails: {str(e)}"
            )
        
    except Exception as e:
        return ToolResult(
            success=False,
            error=f"Error getting sent emails by date range: {str(e)}"
        )
